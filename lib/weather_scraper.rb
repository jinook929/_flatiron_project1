...
class WeatherScraper
  ...
  def self.get_weather_info(lat, long)
    ...
    ### trying area name: 
    # area = HTTParty.get("https://maps.googleapis.com/maps/api/geocode/json?latlng=#{lat},#{long}&sensor=true&key=YOUR_KEY")["plus_code"]["compound_code"].split(/[[:space:]]/)
    # area.shift
    # area = area.join(" ")
    # Return weather info hash
    Hash.new.tap { |hash|
      ### trying area name: hash[:area] = area
